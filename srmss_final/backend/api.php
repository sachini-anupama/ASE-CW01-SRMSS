...
function report_payload(string $type, ?string $from, ?string $to): array
{
    ...
    $where = [];
    $params = [];
    if ($from) { $where[] = 'DATE(s.EstimatedStartTime) >= :from'; $params[':from'] = $from; }
    if ($to) { $where[] = 'DATE(s.EstimatedStartTime) <= :to'; $params[':to'] = $to; }
    ...
    $completed = array_sum(array_map(fn($r) => (int) $r['completed'], $rows));
    $completion = $totalTrips ? round(($completed / $totalTrips) * 100, 1) : 0;

    $lines = ["Report Type: {$type}", 'Generated: ' . date('Y-m-d H:i'), "Total Trips: {$totalTrips}", "Completed Trips: {$completed}", "Completion Rate: {$completion}%"];
    foreach ($rows as $row) {
        $rate = (int) $row['trips'] ? round(((int) $row['completed'] / (int) $row['trips']) * 100, 1) : 0;
        $lines[] = "{$row['label']}: {$row['trips']} trips, {$row['completed']} completed, {$rate}%";
    }

    return [
        'title' => $type ?: 'Route Performance',
        'summary' => [
...
{
    $content = "BT /F1 18 Tf 50 780 Td (" . pdf_text($title) . ") Tj /F1 10 Tf 0 -28 Td ";
    foreach ($lines as $line) {
        $content .= "(" . pdf_text((string) $line) . ") Tj 0 -16 Td ";
    }
    $content .= 'ET';
    $objects = [];
