(function () {
  ...
  function renderReport(data) {
    ...
    if (stats.length >= 4) {
      ...
      stats[2].textContent = state.schedules.filter((s) => s.status === 'Delayed').length;
    ...
    tbody.innerHTML = data.rows.map((row) => {
      ...
      return `<tr><td>${escapeHtml(row.label)}</td><td>${trips}</td><td>${completed}</td><td><strong>${rate}%</strong></td></tr>`;
    }).join('') || emptyRow(4);
  }
  ...
  function initGpsMap() {
    ...
    STOP_DATA.forEach((stop) => {
      L.marker([stop.lat, stop.lng]).addTo(gpsMap).bindPopup(`<strong>${escapeHtml(stop.name)}</strong>`);
    });
  }
  ...
  function drawTripRoute(trip) {
    ...
    const coords = names.map(findStop).filter(Boolean);
    if (coords.length < 2) {
